...
df <- read_excel("data/saad-digi-marketing-dta.xlsx") |>
  ...
  mutate(
    ...
    education = case_when(
      ...
      str_detect(
        education,
        "elementary grad|elementray grad"
      ) ~ "Elementary graduate",
    ...
    years_established = if_else(
      str_detect(years_established, "3-5"),
      "3-5 yrs",
      years_established
    ),
    value_adding = if_else(
      str_detect(value_adding_processed_products_specify, "No"),
      "No",
      "Yes"
    ),
...
### statements
lkrt_statements <-
  read_excel("data/saad-digi-marketing-dta.xlsx", 2) |>
  clean_names() |>
...
lkrt_dta <-
  ...
  mutate(
    ho_factor = case_when(
      ...
      str_detect(
        factor,
        "digital skills|managerial support"
      ) ~ "Organizational support",
      str_detect(factor, "government support") ~ "Environmental barriers",
      TRUE ~ factor
    )
  )
