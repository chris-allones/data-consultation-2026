...
Copies of the Tables and Figures presented in this report are available using the links below.
:::

<ol>

<li><a href="https://docs.google.com/spreadsheets/d/1pAOp4eKFY0QcAsY8W4LecK1U9M7V-9NIK3BbEuCksP8/edit?usp=sharing" target="_blank" style="text-decoration: none">Table</a></li>

<li><a href="https://drive.google.com/drive/folders/13jA3020wpa2mb4zj4vk_yN750etwtqC4?usp=sharing" target="_blank" style="text-decoration: none">Plots</a></li>

<li><a href="https://drive.google.com/drive/folders/175mFKLYBMw2ZY_U-otuGEeRbPxpi5JrH?usp=sharing" target="_blank" style="text-decoration: none">Google Drive</a></li>
...
```{r}
## need cleaning
plt_respondent_faculty <- 
  socio_demo_dta |> 
  count(sex, faculty) |> 
  group_by(sex) |>
  mutate(
    pct = n / sum(n),
    pct_lab = str_c(round(pct, 2) * 100, "%", "(n=", n, ")"),
    faculty = str_wrap(faculty, 50),
    faculty = reorder_within(faculty, n, sex),
  ) |>
  ungroup() |> 
  ggplot(aes(pct, faculty, fill = sex)) +
  geom_col(width = 0.7) +
  geom_text(aes(label = pct_lab), hjust = -0.1) +
  scale_y_reordered() +
  scale_x_continuous(limits = c(0, 0.75), labels = percent_format()) +
  facet_wrap(~ sex, scales = "free", ncol = 1) +
  theme(plot.title = element_text(hjust = 0.5)) +
  custom_theme +
  labs(
    title = "Distribution of respondents by Sex across Faculty",
    fill = NULL,
    y = NULL,
    fill = element_blank()
  )
...
## saving plot
ggsave(
  plot = plt_respondent_faculty,
  filename = "plot/respondent_faculty.jpg",
  unit = "in",
  width = 10,
  height = 9,
  dpi = 400
...
## display plot
knitr::include_graphics("plot/respondent_faculty.jpg")
```
...
```{r}
summary_boot_results$bootstrapped_loadings |> 
  as.data.frame() |> 
  rownames_to_column("Factors") |> 
  mutate(across(is.numeric, ~ round(.x, 3))) |> 
  as.tibble() |> 
