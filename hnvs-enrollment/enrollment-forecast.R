...
cagr_ex <- (last_value_ex /
  enroll_ex$enrolment[enroll_ex$year == min(enroll_ex$year)])^(1 /
  (nrow(enroll_ex) - 1)) -
  1
...
scenario_moderate <- tibble(
  year = (last_year_ex + 1):(last_year_ex + n_years)
) |>
...
p <- ggplot() +
  ...
  scale_x_continuous(
    breaks = seq(min(enroll_ex$year), max(scenarios_all$year), by = 2)
  ) +
  labs(
    x = "Year",
    y = "Enrolment (JHS & SHS)",
    ...
    subtitle = "Note: damped scenario assumes decline rate tapers toward 0 over time"
  ) +
  theme(
...
## saving plot
ggsave("hnvs-enrollment/enrolment_scenario_plot.png", plot = p, width = 9, height = 6, dpi = 300)
