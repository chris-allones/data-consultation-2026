...
df <- read_excel(
  ...
  mutate(location = if_else(str_detect(address, "Bohol|bohol"), "Bohol", "Leyte")) |> 
  mutate(
  ...
  mutate(
    education_attainment = factor(education_attainment,
                                  levels = c("Elementary level", "Elementray graduate", "High school level", "High school graduate", "College level", "Vocational", "College graduate", "Graduate studies", "ALS"))
  ) |> 
  mutate(
...
df |> glimpse()
df |> View()
df |> count(location, address)
df |> 
   |> 
  count(education_attainment)
