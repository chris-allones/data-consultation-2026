...
aware_rice_based_dta <-
  ...
  mutate(
    delicacy = case_when(
      str_detect(delicacy, "budbod") ~ "budbod (suman malagkit)",
...
taste_rice_based_dta <-
  ...
  pivot_longer(everything(), names_to = "delicacy", values_to = "rating") |>
  count(delicacy, rating) |>
  na.omit() |>
  group_by(delicacy) |>
  mutate(pct = n / sum(n)) |>
...
taste_snacks_process_data <-
  ...
  pivot_longer(everything(), names_to = "delicacy", values_to = "rating") |>
  mutate(
    delicacy = str_remove_all(delicacy, "snacks_processed_products_taste_")
  ) |>
  mutate(delicacy = str_replace_all(delicacy, "_", " ")) |>
  count(delicacy, rating) |>
  na.omit() |>
...
taste_beverage_dairy_dta <-
  local_food_dta |>
  select(
    starts_with("dairy_specialty_products_taste"),
    starts_with("beverage_taste")
  ) |>
  pivot_longer(everything(), names_to = "delicacy", values_to = "rating") |>
  mutate(
    delicacy = str_remove_all(delicacy, "dairy_specialty_products_taste_")
  ) |>
  mutate(delicacy = str_replace_all(delicacy, "_", " ")) |>
  count(delicacy, rating) |>
  na.omit() |>
...
reco_local_delicacy_dta <-
  ...
  rename(
    "recommend" = x2_would_you_like_to_recommend_baybay_s_local_food_to_tourists
  ) |>
  mutate(
    recommend = if_else(recommend == "Defnitely", "Definitely", recommend)
  ) |>
  count(recommend) |>
  na.omit() |>
  mutate(pct = n / sum(n)) |>
...
likert_dta <-
  local_food_dta |>
  select(pm1:ab5) |>
  pivot_longer(everything(), names_to = "indicator", values_to = "rating") |>
...
plot_factor <-
  function(factor_name) {
    likert_dta |>
      ...
      scale_fill_manual(
        values = c("#081c15", "#333d29", "#b6ad90", "#a68a64", "#7f4f24")
      ) +
      guides(
